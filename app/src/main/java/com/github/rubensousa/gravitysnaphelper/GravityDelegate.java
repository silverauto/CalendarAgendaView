    ...
    private View findStartView(RecyclerView.LayoutManager layoutManager,
        ...
        if (layoutManager instanceof LinearLayoutManager) {
            ...
            if (visibleWidth > 0.5f && !endOfList) {
                ...
                View nextRowChild = null;
                int firstChildStart = helper.getDecoratedStart(child);
                for(int i=1;i<layoutManager.getChildCount();i++) {
                    nextRowChild = layoutManager.findViewByPosition(firstChild + i);
                    // For GridLayoutManager, we need to find the first child in the next row
                    if(firstChildStart != helper.getDecoratedStart(nextRowChild)) {
                        break;
                    }
                }
                return nextRowChild;
